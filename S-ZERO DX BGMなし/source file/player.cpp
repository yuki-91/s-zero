...
//ターボ
int  CX_model::turbo_count_left = 0;
int  CX_model::turbo_speed_count_left = 0;
int  CX_model::turbo_count_right = 0;
int  CX_model::turbo_speed_count_right = 0;

float  CX_model::handle_left = 0.0f;
float  CX_model::handle_right = 0.0f;
	   
float  CX_model::handle_left_drift = 0.0f;
float  CX_model::handle_right_drift = 0.0f;
	   
float  CX_model::force_left = 0.0;
float  CX_model::force_right = 0.0;

bool  CX_model::drift_state_left = false;
bool  CX_model::drift_state_right = false;
...
{
	//ドリフトジャンプ
	/*if (MyInputGamepad::GetButtonTrigger(MYGAMEPAD_BUTTON_RIGHT_SHOULDER) && !CJump::HitPlayer() ||  GetKeyboardTrigger(DIK_J) && !CJump::HitPlayer())
	{
	if (bJump == false)
	{
	if (OnField)
	...
	//if (bUp_d  && bJump)
	//{
	//	fJump += VALUE_GRAVITY_DRIFT * 1.05;
	//	g_pos.y += VALUE_GRAVITY_DRIFT * 1.05;
