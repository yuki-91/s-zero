...
#define POSE "data/texture/pause.png"
...
void CFade::InitFade(void)
{
	...
	if(FAILED(hr))
	{
		MessageBox(NULL, "読み込めません02", "エラー", MB_OK);
		return ;
	}
...
void CFade::CreateFade(int count, bool out, bool pose)
{
	...
	VERTEX_FADE fade[] =
	{
		{D3DXVECTOR3( 0.0f,		y, 10.0f), 1.0f, D3DCOLOR_RGBA(color, color, color, count), D3DXVECTOR2(0, 1)},
		{D3DXVECTOR3( 0.0f,  0.0f, 10.0f), 1.0f, D3DCOLOR_RGBA(color, color, color, count), D3DXVECTOR2(0, 0)},
		{D3DXVECTOR3(	 x,		y, 10.0f), 1.0f, D3DCOLOR_RGBA(color, color, color, count), D3DXVECTOR2(1, 1)},
		{D3DXVECTOR3(	 x,	 0.0f, 10.0f), 1.0f, D3DCOLOR_RGBA(color, color, color, count), D3DXVECTOR2(1, 0)},
	};
